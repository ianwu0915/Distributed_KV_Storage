...
/**
 * This class contains the logic for handling incoming RPCs from other Raft nodes.
 */
public class RaftConsensusServiceImpl implements RaftConsensusService {
    ...
    /**
     * The preVote RPC is used by a candidate to gather last minute votes before starting a new election.
     * It is similar to the requestVote RPC, but the candidate does not update its term number or grant its vote.
     * This method are for the follower to handle the preVote request send by the candidate.
     * @param request
     * @return
     */
    @Override
    public RaftProto.VoteResponse preVote(RaftProto.VoteRequest request) {
        ...
        try {
            ...
            // If the server is not in the configuration, return the response that the vote is not granted.
            if (!ConfigurationUtils.containsServer(raftNode.getConfiguration(), request.getServerId())) {
                return responseBuilder.build();
            }

            // If the term of the request is less than the current term, return the response that the vote is not granted.
